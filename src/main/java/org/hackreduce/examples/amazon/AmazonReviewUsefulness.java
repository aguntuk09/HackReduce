...
public class AmazonReviewUsefulness  extends org.hackreduce.examples.RecordCounter {
	public static class RecordCounterMapper extends AmazonReviewMapper<Text, FloatWritable> {
		...
		protected void map(AmazonReviewRecord record, Context context) throws IOException, InterruptedException {
			if (record.getReviewDate() == null || record.getTotalCount() == 0) return;
			cal.setTime(record.getReviewDate());
			context.write(new Text("" + record.getRating())
	...
	public static void main(String[] args) throws Exception {
		int result = ToolRunner.run(new Configuration(), new AmazonReviewUsefulness(), args);
		System.exit(result);
	}
}
